...
as is done by the World Health Organization.
...
We can see that decline slows down after creating 4 clusters, and therefore
we will set $k = 4$.
...
For this purpose I will make use of the LASSO, a regression method, that performs
an automatic feature selection. We will try to fit two different models and then
evaluate the both using cross-validation and a test dataset.
...
The most regularized fit has been achieved when lambda equals `r lasso.base.model$lambda.1se`.
The performance
...
Based on the performance of this model on the test dataset, we can say that it is better at making
predictions than our previous model.
...
women, which we have also tested using a t-test. We also revealed that there is a difference
in suicide rates between different age categories, and that mainly in Europe and Asia,
the suicide risk gets larger with the age a person. Last but not least, suicide rates within
many countries were strongly correlated with the GDP per capita of the country.

We found that Europe has the highest suicide rate out of all of the continents, with Russia
...
able to find, given that we would expand our dataset with new data. It would be interesting
to check whether there is a relationship of a suicide rate within a country and the country's
happiness index for example. I would also be interested in the connection between a country's
