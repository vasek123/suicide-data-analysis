...
# perc_99 <- quantile(ml.data$suicide_rate, probs = 0.95)
# ml.data <- ml.data %>% select(selected.columns) %>% filter(suicide_rate < perc_99)
...
X <- model.matrix(formula, data = ml.data.encoded)
y <- ml.data$suicide_rate
...
# Prepare the data pre-processor
# preProcValues <- preProcess(train.data, method = c("center", "scale"))
# train.data <- predict(preProcValues, train.data)
# test.data <- predict(preProcValues, test.data)
